...
// ServicePortsDiscovery is used to receive rpc calls from other nodes
...
// ServiceNodePolling is used to receive rpc calls from other nodes
...
// ServiceNodeRegister is used to receive rpc calls from other nodes
func (c *Cluster) ServiceNodeRegister(ctx context.Context, in *scalezillapb.ServiceNodeRegisterRequest) (*scalezillapb.ServiceNodeRegisterReply, error) {
	responseChan := make(chan RPCResponse, 1)
	select {
	case c.rpcServiceNodeRegisterChanReq <- RPCRequest{
	...
	select {
	case response := <-responseChan:
		return response.Response.(*scalezillapb.ServiceNodeRegisterReply), response.Error
